...
// Model for Adres entity
    models.Adres = Backbone.Model.extend({
        urlRoot: "http://localhost:8080/Workshop7/webresources/entity.adres/",
        idAttribute: 'adresId',
        defaults: {
            huisnummer: "",
            postcode: "",
        ...
        toViewJson: function () {
            ...
            result.displayName = this.get('adresId');
            return result;
        },
        isNew: function () {
        ...
        sync: function (method, model, options) {
            ...
            if (method === 'create') {
                options.url = 'http://localhost:8080/Workshop7/webresources/entity.adres/';
            }
    ...
    // Collection class for Adres entities
    models.AdresCollection = Backbone.Collection.extend({
        model: models.Adres,
        url: "http://localhost:8080/Workshop7/webresources/entity.adres/",
        sync: function (method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function (jqXHR, textStatus, errorThrown) {
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.AdresCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-adres-list-itemis template identifier for item
                        templateName: '#tpl-adres-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.adres = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.adres,
                    // tpl-adres-details is template identifier for chosen model element
                    templateName: '#tpl-adres-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.Adres();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-adres-details is a template identifier for chosen model element
                templateName: '#tpl-adres-details',
