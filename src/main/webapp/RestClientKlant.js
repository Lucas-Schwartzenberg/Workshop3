...
// Model for Klant entity
    models.Klant = Backbone.Model.extend({
        urlRoot: "http://localhost:8080/Workshop7/webresources/entity.klant/",
        idAttribute: 'klantId',
        defaults: {
            achternaam: "",
            tussenvoegsel: "",
        ...
        toViewJson: function () {
            ...
            result.displayName = this.get('klantId');
            return result;
        },
        isNew: function () {
        ...
        sync: function (method, model, options) {
            ...
            if (method === 'create') {
                options.url = 'http://localhost:8080/Workshop7/webresources/entity.klant/';
            }
    ...
    // Collection class for Klant entities
    models.KlantCollection = Backbone.Collection.extend({
        model: models.Klant,
        url: "http://localhost:8080/Workshop7/webresources/entity.klant/",
        sync: function (method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function (jqXHR, textStatus, errorThrown) {
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.KlantCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-klant-list-itemis template identifier for item
                        templateName: '#tpl-klant-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.klant = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.klant,
                    // tpl-klant-details is template identifier for chosen model element
                    templateName: '#tpl-klant-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.Klant();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-klant-details is a template identifier for chosen model element
                templateName: '#tpl-klant-details',
